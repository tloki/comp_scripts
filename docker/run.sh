...
# OpenBLAS target variable, from list available at link:
# https://github.com/xianyi/OpenBLAS/blob/develop/TargetList.txt
# (or look for specified branch)
# this needs to be set for now
# TODO: autodetect!
OBTARGET=HASWELL

# custom release versions
# if unset (comment out) - latest release will be selected
...
# for Ubuntu, as of 18.04, 7 is preffered 
# TODO: fix custom
# TODO: autodetect
GV=7

# set march for compilation, leave unset to detect
# MARCH=
...
# deprecated, but more stable and gitlab compatible?
# sometimes returns non-stable versions (i.e. release candidates or so)
...
# TODO: fix .git and trailing / anomaly
...
if [ -z ${var+x} ]
then
    echo "MARCH variable is unset, detecting march for set compiler version"
    MARCH=`gcc -c -Q -march=native --help=target | grep march | grep -io  "\s[a-z]\+" | grep -io "[a-z]\+"`
    echo "autodetected MARCH=$MARCH"
else
    echo "MARCH is user set to '$MARCH'"
fi

echo "march parameter set to $MARCH"
...
export USE_THREAD=1
export NO_WARMUP=0
export TARGET=$OBTARGET
export NO_PARALLEL_MAKE=0
export MAKE_NB_JOBS=$NTHREADS
export PREFIX=/opt/OpenBLAS
